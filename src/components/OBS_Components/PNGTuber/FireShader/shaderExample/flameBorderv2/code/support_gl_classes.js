...
class Rect_with_hole {
  ...
  #update_vertices() {
    ...
    for (let i = 0; i < hole_positions["positions"].length; i += 2) {
      ...
      let up_pos = is_intersect_ray_segment(
        circle_center,
        direction,
        lu_rect,
        ru_rect
      );
      let right_pos = is_intersect_ray_segment(
        circle_center,
        direction,
        ru_rect,
        rd_rect
      );
      let down_pos = is_intersect_ray_segment(
        circle_center,
        direction,
        ld_rect,
        rd_rect
      );
      let left_pos = is_intersect_ray_segment(
        circle_center,
        direction,
        lu_rect,
        ld_rect
      ...
      let m_up_pos = is_intersect_ray_segment(
        circle_center,
        direction,
        m_lu_rect,
        m_ru_rect
      );
      let m_right_pos = is_intersect_ray_segment(
        circle_center,
        direction,
        m_ru_rect,
        m_rd_rect
      );
      let m_down_pos = is_intersect_ray_segment(
        circle_center,
        direction,
        m_ld_rect,
        m_rd_rect
      );
      let m_left_pos = is_intersect_ray_segment(
        circle_center,
        direction,
        m_lu_rect,
        m_ld_rect
...
class Uniform_binder {
  ...
  constructor() {
    ...
    this.#uniform_functors.set("uniformMatrix2fv", this.#set_uniformMatrix2fv);
    this.#uniform_functors.set("uniformMatrix3fv", this.#set_uniformMatrix3fv);
    this.#uniform_functors.set("uniformMatrix4fv", this.#set_uniformMatrix4fv);
  }
  ...
  set_uniform(
    ...
    if (values.every(value => is_number(value))) {
      uniform_type =
        "uniform" + values_count + (convert_int_to_float ? "f" : "i");
      ...
      if (values[0].every(value => is_integer(value) || is_boolean(value))) {
        uniform_type =
          "uniform" + values[0].length + (convert_int_to_float ? "f" : "i");
...
class Shader {
  ...
  use() {
    ...
    if (
      this.#gl.getParameter(this.#gl.CURRENT_PROGRAM) === this.#shader_program
    ) {
      return;
    }
  ...
  #set_uniform(
    ...
    const uniform_location = this.#get_cached_uniform_location(uniform_name);

    if (uniform_location === null) {
      console.error(
        `Shader [${this.#shader_program}]` +
    ...
    this.#uniform_binder.set_uniform(
      this.#gl,
      uniform_location,
      convert_int_to_float,
      convert_array_to_matrix,
  ...
  #compile_sources(vertex_source, fragment_source) {
    const vertex_shader = this.#compile_shader(
      vertex_source,
      this.#gl.VERTEX_SHADER
    );

    const fragment_shader = this.#compile_shader(
      fragment_source,
    ...
    this.#shader_program = this.#compile_program(
      vertex_shader,
      fragment_shader
    );

    this.#status = this.#check_shaders(this.#shader_program, "shader program");
...
class Texture_loader {
  ...
  update_texture(
    ...
    if (is_string(data)) {
      this.#update_texture_from_file(
        texture,
        data,
        on_update,
        is_need_use_new_size,
        x_offset,
        y_offset
      );
    } else if (is_uint8array(data)) {
      this.#update_texture_from_array(
        texture,
        data,
        on_update,
        is_need_use_new_size,
